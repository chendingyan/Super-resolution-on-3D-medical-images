...
import os
from lib.model import data_loader, generator, SRGAN, test_data_loader, run_data_loader, save_images
from lib.ops import *
import math
import time
import numpy as np
...
Flags.DEFINE_string('output_dir', None, 'If your mode is training, it means the output directory of your model checkpoint; If your mode is running, it means your output result directory')
Flags.DEFINE_string('summary_dir', None, 'If your mode is training, it means the output directory of your pre-trained model')
Flags.DEFINE_string('mode', 'train', 'The mode of the model train, test.')
Flags.DEFINE_string('checkpoint', None, 'If provided, the weight will be restored from the provided checkpoint')
Flags.DEFINE_boolean('pre_trained_model', False, 'If set True, the weight will be loaded but the global_step will still be 0. That means you start a new training process again. If set False, you are going to continue the training from the checkpoint you choose. The global_step will be initialized from the checkpoint. ')
Flags.DEFINE_string('pre_trained_model_type', 'SRGAN', 'The type of pretrained model')
Flags.DEFINE_boolean('is_training', True, 'If you set to true, it means training, otherwise testing')
Flags.DEFINE_string('vgg_ckpt', './vgg19/vgg_19.ckpt', 'path to checkpoint file for the vgg19')
Flags.DEFINE_string('task', 'SRGAN', 'The task: SRGAN')
# The data preparing operation
Flags.DEFINE_integer('batch_size', 16, 'Batch size of the input batch, each training takes batchsize number of samples in the training set')
...
Flags.DEFINE_integer('crop_size', 24, 'The size of trimming the training set image, the image after crop will be 24*24')
...
Flags.DEFINE_float('beta', 0.9, 'The beta parameter for the Adaptive Moment Estimation(Adam) Optimizer(we don’t use SGD)')
...
if FLAGS.mode == 'test':
    ...
    # Define the weight initiallizer (When you run the SR process, we only need to restore the weight of the generator)
...
# the run mode (just perform super resolution on the input image)
elif FLAGS.mode == 'run':
    # Check the checkpoint
    if FLAGS.checkpoint is None:
        raise ValueError('The checkpoint file is needed to performing the test.')
    ...
    # Declare the test data reader
    run_data = run_data_loader(FLAGS)
    ...
    with tf.Session(config=config) as sess:
        ...
        max_iter = len(run_data.inputs)
        print('Evaluation starts!!')
        for i in range(max_iter):
            input_im = np.array([run_data.inputs[i]]).astype(np.float32)
            path_lr = run_data.paths_LR[i]
...
# The training mode
    ...
    # If you initial your weight from other SRGAN checkpoint, var_list2 need to contain discriminator weights.
    if FLAGS.task == 'SRGAN':
    ...
    with sv.managed_session(config=config) as sess:
        if (FLAGS.checkpoint is not None) and (FLAGS.pre_trained_model is False):
            ...
            # saver.restore(sess, "./experiment_SRGAN_VGG54/model-60000")
        ...
        for step in range(max_iter):
            ...
            if ((step + 1) % FLAGS.display_freq) == 0:
                ...
                print("progress  iteration %d  step %d  image/sec %0.1f  remaining %dm" % (train_epoch, train_step, rate, remaining / 60))
                if FLAGS.task == 'SRGAN':
